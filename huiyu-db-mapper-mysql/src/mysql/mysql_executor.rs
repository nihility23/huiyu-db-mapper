...
impl Executor for MysqlSqlExecutor {
        ...
        Q: FnOnce(Vec<T>) -> Result<R, DatabaseError> + Send + 'static
    {
        ...
        spawn_blocking(move || {
            let stat = conn.lock().map_err(|e| DatabaseError::AccessError(e.to_string()))?.prep(sql).map_err(|e| DatabaseError::ExecuteError(e.to_string()))?;
            let mut vec = Vec::new();
            for param in params.iter() {
                vec.push(param_value_to_value(param)?);
            }
            let res = conn.lock().map_err(|e| DatabaseError::AccessError(e.to_string()))?.exec_map(stat, Params::Positional(vec),|row: Row|{
    ...
    async fn execute(&self, conn: Arc<Mutex<Self::Conn>>, sql: &str, params: &Vec<ParamValue>) -> Result<u64, DatabaseError> {
        ...
        let res:Option<Value> = conn.lock().map_err(|e| DatabaseError::AccessError(e.to_string()))?.exec_first(sql, Params::Positional(vec)).map_err(|e| DatabaseError::ConvertError(e.to_string()))?;
        if res.is_none() {
            return Ok(0);
        }
    ...
    async fn start_transaction(&self) -> Result<(), DatabaseError> {
        ...
        conn.lock().map_err(|e| DatabaseError::AccessError(e.to_string()))?.exec_first::<Value, &str, mysql::Params>("BEGIN", Params::Positional(vec![])).map_err(|e| DatabaseError::ExecuteError(e.to_string()))?;
        Ok(())
    }

    async fn commit(&self) -> Result<(), DatabaseError> {
        let conn = self.get_conn_ref()?;
        conn.lock().map_err(|e| DatabaseError::AccessError(e.to_string()))?.exec_first::<Value, &str, mysql::Params>("COMMIT", Params::Positional(vec![])).map_err(|e| DatabaseError::ExecuteError(e.to_string()))?;
        Ok(())
    }

    async fn rollback(&self) -> Result<(), DatabaseError> {
        let conn = self.get_conn_ref()?;
        conn.lock().map_err(|e| DatabaseError::AccessError(e.to_string()))?.exec_first::<Value, &str, mysql::Params>("ROLLBACK", Params::Positional(vec![])).map_err(|e| DatabaseError::ExecuteError(e.to_string()))?;
        Ok(())
    }
