...
// 查询工具类分为几种情况
// 返回个数：insert_batch,update,update_by_key,delete,delete_by_key,delete_by_keys
// 返回单个实体(E):select_one,select_by_key
// 返回多个实体(Vec<E>):select,select_by_keys
// 返回分页(Page<E>):select_page
// 返回主键：insert_one
...
impl DbManager {
    ...
    pub(crate) fn query_by_key<T,F>(table_name:&str,id:u64,f:F)->Result<Option<T>,CustomError> where F:FnMut(&Row<'_>) -> rusqlite::Result<T>,T:Serialize{
        let mut param_vec = Vec::new();
        param_vec.push(ParamType::U64(id));
        DbManager::query_one(format!("select * from {} where id = ?",&table_name).as_str(), &param_vec, f, |t:&mut Vec<T>|{
            Ok(Some(t.pop().unwrap()))
        })
    }

    pub(crate) fn delete_by_key(table_name:&str,id:u64)->Result<(),CustomError>{
        let mut param_vec = Vec::new();
        param_vec.push(ParamType::U64(id));
        DbManager::exec(format!("delete from {} where id = ?",&table_name).as_str(), &param_vec)
    }

    pub(crate) fn query_page<P,QF,T,F>(base_query_sql: &str,current_page:u32, page_size:u32, p: &P, qf:QF,f:F)->Result<PageResult<T>,CustomError> where P:Serialize,QF:Fn(&P)->(Vec<String>,Vec<ParamType>),T:Serialize + std::fmt::Debug,F: FnMut(&Row<'_>) -> rusqlite::Result<T>{
        ...
        query_param_value_vec.push(ParamType::U32(page_size));
        query_param_value_vec.push(ParamType::U32((current_page-1) * page_size));
    ...
    pub(crate) fn query_count(sql:&str, params: &Vec<ParamType>)->Result<u64,CustomError> {
    ...
    pub(crate) fn insert_one(sql:&str, params: &Vec<ParamType>)->Result<u64,CustomError>{
        let res = Self::query_one(sql,params,|row|{
            Ok(row.get::<_,u64>(0)?)
        },|t|{
            Ok(Some(*t.get(0).unwrap() as u64))
    ...
    pub(crate) fn query_vec<F,T>(sql:&str, params: &Vec<ParamType>, f:F ) -> Result<Vec<T>,CustomError> where T:Serialize, F: FnMut(&Row<'_>) -> rusqlite::Result<T> {
    ...
    pub(crate) fn query_one<F,T,K,Q>(sql:&str, params: &Vec<ParamType>, f:F, k:K ) -> Result<Option<Q>,CustomError> where T:Serialize, F: FnMut(&Row<'_>) -> rusqlite::Result<T>, K: Fn(&mut Vec<T>) -> Result<Option<Q>,CustomError> {
