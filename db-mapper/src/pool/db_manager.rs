...
impl<M: Send + Sync + 'static> DbManager<M> {
    ...
    /// 注册新的数据库实例（可以多次调用）
    pub fn register<F>(config: &DbConfig, factory: F) -> Result<Arc<Self>, Box<dyn Error>>
    where
        F: Fn(&DbConfig) -> M + Sync + Send + 'static,
    {
        ...
        if registry.contains::<M>(&config.name) {
            return Err(format!("Database instance with name '{}' already exists", config.name).into());
        }
